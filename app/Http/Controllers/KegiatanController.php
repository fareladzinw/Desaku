...
class KegiatanController extends Controller
{
    ...
    public function showKegiatanFilter(Request $request){
        $id = $request->input('id');
        ...
        ->when(Input::has('tanggal'), function ($query) use ($tanggal) {
        ...
        if($data){
            $res['message'] = "success";
            $res['detail'] = "Berhasil mendapatkan data Kegiatan";
            $res['data'] = $data;
            return response($res,200);
        }
        else{
            $res['message'] = "failed";
            $res['detail'] = "Gagal Mendapat Data Kegiatan";
    ...
    public function showBerita(){
        ...
        if($data){
            $res['message'] = "success";
            $res['detail'] = "Data Semua Kegiatan";
            $res['data'] = $data;
            return response($res,200);
        }
        else{
